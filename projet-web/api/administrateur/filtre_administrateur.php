...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            switch (true) {
                case ((!empty($donnees->Nom) and !empty($donnees->Prenom)) and
                    $donnees->Nom == $admin['Nom'] and
                    $donnees->Prenom == $admin['Prenom']):
                    $tableauadministrateur['administrateur'][] = $admin;
                    break;
                case ((!empty($donnees->Nom) and empty($donnees->Prenom)) and
                    $donnees->Nom == $admin['Nom']):
                    $tableauadministrateur['administrateur'][] = $admin;
                    break;
                case ((empty($donnees->Nom) and !empty($donnees->Prenom)) and
                    $donnees->Prenom == $admin['Prenom']):
                    $tableauadministrateur['administrateur'][] = $admin;
                    break;
                case ((empty($donnees->Nom) and empty($donnees->Prenom))):
                    $tableauadministrateur['administrateur'][] = $admin;
                    break;
            }
        }
