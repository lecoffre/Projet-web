...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $donnees->Evaluation_des_stagiaires;
        $donnees->Confiance_du_Pilote_de_promotion;
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            switch (true) {
                case (!empty($donnees->Secteur_activite) and !empty($donnees->Evaluation_des_stagiaires) and !empty($donnees->Confiance_du_Pilote_de_promotion) and !empty($donnees->Nom_entreprise) and
                    $donnees->Secteur_activite == $entre['Secteur_activite'] and
                    $donnees->Evaluation_des_stagiaires <= $entre['Evaluation_des_stagiaires'] and
                    $donnees->Confiance_du_Pilote_de_promotion <= $entre['Confiance_du_Pilote_de_promotion'] and
                    $donnees->Nom_entreprise == $entre['Nom_entreprise']):
                ...
                case (empty($donnees->Secteur_activite) and empty($donnees->Evaluation_des_stagiaires) and empty($donnees->Confiance_du_Pilote_de_promotion) and !empty($donnees->Nom_entreprise) and
                    ...
                    $tableauentreprise['entreprise'][] = $entre;
                    break;
                case (empty($donnees->Secteur_activite) and empty($donnees->Evaluation_des_stagiaires) and empty($donnees->Confiance_du_Pilote_de_promotion) and empty($donnees->Nom_entreprise)):
                    $tableauentreprise['entreprise'][] = $entre;
                    break;
            }
        }
